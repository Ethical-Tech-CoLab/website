...
import { ercfReport } from "@/content/publications/ercf";
...
export const metadata: Metadata = {
  ...
  description:
    "An Ethical Tech CoLab report on a decision support prototype that costs a civilian evacuation against the cost of staying, scoring seven weighted dimensions and calculating the day on which one overtakes the other.",
};

export default function ErcfPage() {
  return (
    <>
      {/* Hero */}
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-24">
          ...
          <Reveal delay={0.05}>
            <h1 className="mt-6 fluid-hero font-heading uppercase leading-[0.9]">
              The Evacuation <span className="display-em">Risk and Cost</span>{" "}
              Framework
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
          ...
          <Reveal delay={0.15}>
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-accent">
              <span className="font-semibold">{ercfReport.org}</span>
              <span aria-hidden className="text-muted">
                ·
              </span>
              <span>{ercfReport.advisor}</span>
              {ercfReport.date && (
                <>
                  <span aria-hidden className="text-muted">
                    ·
                  </span>
                  <span>{ercfReport.date}</span>
                </>
              )}
            </div>
          ...
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              >
                Open the live framework <span aria-hidden>↗</span>
              </a>
              <a
                href={ercfReport.repoUrl}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <ReportPdfLink slug="ercf" />
              <ReportBookLink slug="ercf" title={ercfReport.title} />
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        ...
        <Reveal delay={0.05}>
          ...
          >
            ...
            <ol className="mt-4 grid gap-2 sm:grid-cols-2">
              {ercfReport.sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
        ...
        <section id="references" className="mt-16 scroll-mt-24">
          ...
          <ol className="mt-8 space-y-4 text-sm leading-relaxed text-muted">
            {ercfReport.citations.map((cite, i) => (
        ...
        <section className="mt-16 rounded-2xl border border-border bg-card p-6">
          <p className="text-sm leading-relaxed text-muted">
            ...
            prototype is for academic demonstration only. Its outputs are
            indicative and are not a substitute for operational decision-making,
            legal advice, or assessment by qualified humanitarian and IHL
            professionals. All cost figures represent logistics estimates. The
            tool does not place a monetary value on human life.
          </p>
        </section>

...
export const metadata: Metadata = {
  ...
  description:
    "An Ethical Tech CoLab report on a decision support tool that scores the risk of leaving against the risk of staying across 104 active humanitarian crises, keeping danger and feasibility deliberately apart.",
};

export default function EvacuationInformIndexPage() {
  return (
    <>
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-24">
          <Reveal>
            ...
            >
              ← {evacuationIndexReport.eyebrow}
            </Link>
          </Reveal>
          <Reveal delay={0.05}>
            <h1 className="mt-6 fluid-hero font-heading uppercase leading-[0.9]">
              The Evacuation{" "}
              <span className="display-em">Inform</span> Index
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-5 max-w-2xl font-heading text-2xl uppercase tracking-wide text-muted sm:text-3xl">
              {evacuationIndexReport.subtitle}
            </p>
          </Reveal>
          <Reveal delay={0.15}>
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-accent">
              <span className="font-semibold">{evacuationIndexReport.org}</span>
              <span aria-hidden className="text-muted">
                ·
              </span>
              <span>{evacuationIndexReport.advisor}</span>
              {evacuationIndexReport.date && (
                <>
                  <span aria-hidden className="text-muted">
                    ·
                  </span>
                  <span>{evacuationIndexReport.date}</span>
                </>
              )}
            </div>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
              {evacuationIndexReport.authors}
            </p>
          </Reveal>
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={evacuationIndexReport.liveUrl}
                target="_blank"
              ...
              <a
                href={evacuationIndexReport.repoUrl}
                target="_blank"
                rel="noopener noreferrer"
      ...
      <section className="border-b border-border bg-surface/40">
        <div className="mx-auto grid max-w-6xl gap-px overflow-hidden border-x border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {evacuationIndexReport.stats.map((stat) => (
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        ...
        <Reveal delay={0.05}>
          ...
          >
            ...
            <ol className="mt-4 grid gap-2 sm:grid-cols-2">
              {evacuationIndexReport.sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
        ...
        <section id="references" className="mt-16 scroll-mt-24">
          ...
          <ol className="mt-8 space-y-4 text-sm leading-relaxed text-muted">
            {evacuationIndexReport.citations.map((cite, i) => (
        ...
        <section className="mt-16 rounded-2xl border border-border bg-card p-6">
          <p className="text-sm leading-relaxed text-muted">
            This report describes a research prototype. The Evacuation Inform
            Index has not been validated, carries no institutional mandate, and
            must not be used as the sole basis for any decision affecting the
            movement or safety of civilians.
          </p>
        </section>

...
export const metadata: Metadata = {
  ...
  description:
    "An Ethical Tech CoLab concept report on whether two isolated agents can invent a grounded, auditable language through shared experience alone, with independent cryptographically anchored ledgers and a pre-registered programme of nineteen experiments.",
};

export default function AgenticLanguageDevelopmentPage() {
  return (
    <>
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-24">
          <Reveal>
            ...
            >
              ← {agenticLanguageReport.eyebrow}
            </Link>
          </Reveal>
          <Reveal delay={0.05}>
            <h1 className="mt-6 fluid-hero font-heading uppercase leading-[0.9]">
              Agentic <span className="display-em">Language</span>{" "}
              Development
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-5 max-w-2xl font-heading text-2xl uppercase tracking-wide text-muted sm:text-3xl">
              {agenticLanguageReport.subtitle}
            </p>
          </Reveal>
          <Reveal delay={0.15}>
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-accent">
              <span className="font-semibold">
                {agenticLanguageReport.org}
              </span>
              <span aria-hidden className="text-muted">
                ·
              </span>
              <span>{agenticLanguageReport.advisor}</span>
              {agenticLanguageReport.date && (
                <>
                  <span aria-hidden className="text-muted">
                    ·
                  </span>
                  <span>{agenticLanguageReport.date}</span>
                </>
              )}
            </div>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
              {agenticLanguageReport.authors}
            </p>
          </Reveal>
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={agenticLanguageReport.liveUrl}
                target="_blank"
              ...
              <a
                href={agenticLanguageReport.repoUrl}
                target="_blank"
                rel="noopener noreferrer"
      ...
      <section className="border-b border-border bg-surface/40">
        <div className="mx-auto grid max-w-6xl gap-px overflow-hidden border-x border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {agenticLanguageReport.stats.map((stat) => (
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        ...
        <Reveal delay={0.05}>
          ...
          >
            ...
            <ol className="mt-4 grid gap-2 sm:grid-cols-2">
              {agenticLanguageReport.sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
        ...
        <section id="references" className="mt-16 scroll-mt-24">
          ...
          <ol className="mt-8 space-y-4 text-sm leading-relaxed text-muted">
            {agenticLanguageReport.citations.map((cite, i) => (
        ...
        <section className="mt-16 rounded-2xl border border-border bg-card p-6">
          <p className="text-sm leading-relaxed text-muted">
            This report describes a research concept and a pre-registered
            experiment programme, not a system that has been run. No experiment
            in it has been executed, and no result, capability, or emergent
            language is claimed. Every statement about what agents might do is a
            hypothesis to be tested, or a finding from the published work cited
            in the references. The experimental encodings discussed in Section
            12 are studies of novelty and coordination; none of them is
            production cryptography and none should be represented as such.
          </p>
        </section>

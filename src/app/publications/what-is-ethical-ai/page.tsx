...
import { whatIsEthicalAiReport } from "@/content/publications/what-is-ethical-ai";
...
export const metadata: Metadata = {
  ...
  description:
    "The Ethical Tech CoLab's foundational paper. It traces ethics from the earliest civilizations through international affairs and human rights law to the responsible AI movement, the humanitarian sector, and the UN system, arguing that ethical AI is institutional rather than technical.",
};

export default function WhatIsEthicalAiPage() {
  return (
    <>
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-24">
          <Reveal>
            ...
            >
              ← {whatIsEthicalAiReport.eyebrow}
            </Link>
          </Reveal>
          <Reveal delay={0.05}>
            <h1 className="mt-6 fluid-hero font-heading uppercase leading-[0.9]">
              What Is <span className="display-em">Ethical AI?</span>
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-5 max-w-2xl font-heading text-2xl uppercase tracking-wide text-muted sm:text-3xl">
              {whatIsEthicalAiReport.subtitle}
            </p>
          </Reveal>
          <Reveal delay={0.15}>
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-accent">
              <span className="font-semibold">{whatIsEthicalAiReport.org}</span>
              <span aria-hidden className="text-muted">
                ·
              </span>
              <span>{whatIsEthicalAiReport.advisor}</span>
              {whatIsEthicalAiReport.date && (
                <>
                  <span aria-hidden className="text-muted">
                    ·
                  </span>
                  <span>{whatIsEthicalAiReport.date}</span>
                </>
              )}
            </div>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
              {whatIsEthicalAiReport.authors}
            </p>
          </Reveal>
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap gap-3">
              {whatIsEthicalAiReport.pdfUrl && (
                <a
                  href={whatIsEthicalAiReport.pdfUrl}
                  target="_blank"
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        ...
        <Reveal delay={0.05}>
          ...
          >
            ...
            <ol className="mt-4 grid gap-2 sm:grid-cols-2">
              {whatIsEthicalAiReport.sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
        ...
        <section id="references" className="mt-16 scroll-mt-24">
          <Reveal>
            ...
            <h2 className="mt-2 fluid-h2 font-heading uppercase">Works Cited</h2>
          </Reveal>
          <ol className="mt-8 space-y-4 text-sm leading-relaxed text-muted">
            {whatIsEthicalAiReport.citations.map((cite, i) => (
        ...
        <section className="mt-16 rounded-2xl border border-border bg-card p-6">
          <p className="text-sm leading-relaxed text-muted">
            {whatIsEthicalAiReport.disclaimer}
          </p>
        </section>

...
export const metadata: Metadata = {
  ...
  description:
    "An Ethical Tech CoLab report on rebuilding WarGames (1983) as a browser game whose machine is a real language model, and on what four tracks of Monte Carlo evaluation found about models asked to drive a scenario they can act on.",
};
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-24">
          ...
          <Reveal delay={0.05}>
            <h1 className="mt-6 fluid-hero font-heading uppercase leading-[0.9]">
              The Only <span className="display-em">Winning Move</span>
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
          ...
          <Reveal delay={0.15}>
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-accent">
              <span className="font-semibold">{warGamesReport.org}</span>
              <span aria-hidden className="text-muted">
                ·
              </span>
              <span>{warGamesReport.advisor}</span>
              {warGamesReport.date && (
                <>
                  <span aria-hidden className="text-muted">
                    ·
                  </span>
                  <span>{warGamesReport.date}</span>
                </>
              )}
            </div>
          ...
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              >
                Play the game <span aria-hidden>↗</span>
              </a>
              <a
                href={warGamesReport.repoUrl}
                target="_blank"
                rel="noopener noreferrer"
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        ...
        <Reveal delay={0.05}>
          ...
          >
            ...
            <ol className="mt-4 grid gap-2 sm:grid-cols-2">
              {warGamesReport.sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
        ...
        {/* Standing disclaimer. The artefact dramatises nuclear escalation, so
            this is load-bearing rather than boilerplate. */}
        <section className="mt-16 rounded-2xl border border-border bg-card p-6">
          <p className="text-sm leading-relaxed text-muted">
            ...
            demonstration and teaching. The game dramatises a fictional
            escalation borrowed from a 1983 film. It models no real command and
            control system, no real escalation dynamics, and no real state
            behaviour, and nothing it produces should inform any policy,
            operational, or intelligence judgement. Model measurements are of
            specific model versions under one prompt contract in July 2026.
          </p>
        </section>

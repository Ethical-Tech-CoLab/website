...
import { provenanceSearchReport } from "@/content/publications/provenance-search";
...
export const metadata: Metadata = {
  ...
  description:
    "An Ethical Tech CoLab report on a prototype that assembles an artwork's ownership history from seven free public sources, names the gaps it cannot account for, and scores its own confidence by a fixed published rule.",
};

export default function ProvenanceSearchPage() {
  return (
    <>
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-20 sm:py-24">
          <Reveal>
            ...
            >
              ← {provenanceSearchReport.eyebrow}
            </Link>
          </Reveal>
          <Reveal delay={0.05}>
            <h1 className="mt-6 fluid-hero font-heading uppercase leading-[0.9]">
              Provenance <span className="display-em">Search</span>
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-5 max-w-2xl font-heading text-2xl uppercase tracking-wide text-muted sm:text-3xl">
              {provenanceSearchReport.subtitle}
            </p>
          </Reveal>
          <Reveal delay={0.15}>
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-accent">
              <span className="font-semibold">{provenanceSearchReport.org}</span>
              <span aria-hidden className="text-muted">
                ·
              </span>
              <span>{provenanceSearchReport.advisor}</span>
              {provenanceSearchReport.date && (
                <>
                  <span aria-hidden className="text-muted">
                    ·
                  </span>
                  <span>{provenanceSearchReport.date}</span>
                </>
              )}
            </div>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
              {provenanceSearchReport.authors}
            </p>
          </Reveal>
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={provenanceSearchReport.liveUrl}
                target="_blank"
              ...
              >
                Open the live demonstration <span aria-hidden>↗</span>
              </a>
              <a
                href={provenanceSearchReport.repoUrl}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <ReportPdfLink slug="provenance-search" />
              <ReportBookLink slug="provenance-search" title={provenanceSearchReport.title} />
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="border-b border-border bg-surface/40">
        <div className="mx-auto grid max-w-6xl gap-px overflow-hidden border-x border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {provenanceSearchReport.stats.map((stat) => (
      ...
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        ...
        <Reveal delay={0.05}>
          ...
          >
            ...
            <ol className="mt-4 grid gap-2 sm:grid-cols-2">
              {provenanceSearchReport.sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
        ...
        <section id="references" className="mt-16 scroll-mt-24">
          ...
          <ol className="mt-8 space-y-4 text-sm leading-relaxed text-muted">
            {provenanceSearchReport.citations.map((cite, i) => (
        ...
        <section className="mt-16 rounded-2xl border border-border bg-card p-6">
          <p className="text-sm leading-relaxed text-muted">
            ...
            demonstration only. Its outputs are indicative and are not a
            substitute for professional provenance research, legal advice, or a
            search of the restricted law-enforcement and commercial databases on
            which art-market due diligence relies. A result from this tool is
            not a clearance, a certificate of title, or a finding of any kind.
          </p>
        </section>

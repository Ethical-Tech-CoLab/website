...
export const publications = {
  ...
  items: [
    // Indices 01–04 were umbrella "In preparation" cards — one per research
    // area, each promising a synthesis report that did not exist. Every one of
    // those areas is now covered by published reports below (Evacuation 07–12
    // and 27, Cultural heritage 13–15, Traceability 16, Diplomacy 17), so the
    // placeholders were removed rather than left advertising an empty link.
...
export interface PartnerOrg {
  ...
  logo?: string;
  /**
   * Tile the logo sits on. Defaults to white, which suits most marks. Set to
   * "dark" for brands that only publish a white/light logo, which would
...
export const about = {
    ...
    items: [
      {
        ...
        url: "",
        logo: "/logos/generative-ai-for-good.jpg",
      },
      {
        name: "D_ID",
        about:
  ...
  partners: {
    ...
    items: [
      { name: "100x", about: "", url: "", logo: "/logos/100x.jpg" },
      {
        name: "Apne Aap Women Worldwide",
        about: "",
        url: "https://apneaap.org",
        logo: "/logos/apne-aap-women-worldwide.png",
      },
      { name: "Gaia", about: "", url: "", logo: "/logos/gaia.png" },
      {
      ...
      },
      {
        name: "B3IQ",
        about:
          "US-based AI infrastructure: NVIDIA-powered GPU servers that buyers own, racked and run in a data center in Eugene, Oregon. B3IQ hosts the CoLab's GPU machine, which the cohort reaches over SSH for model work.",
        url: "https://b3iq.org",
        logo: "/logos/b3iq.png",
        // B3IQ only publishes a white wordmark, which the default white tile
        // would swallow.
        logoTile: "dark",
      },
      {
...
export const team = {
  ...
  researchers: [
    ...
    },
    {
      ...
      role: "Applied AI Graduate Researcher",
      term: "Fall 2025",
      linkedin: "https://www.linkedin.com/in/pegi-bracaj/",
      // photo: "/team/pegi.jpg", // TODO: headshot not yet supplied
      slug: "pegi-bracaj",
      bio: "Pegi Bracaj joined the Ethical Tech CoLab with the Fall 2025 cohort, and is a co-author of AI-Powered Research Questions, the cohort's study of AI assistance across the researcher's workflow.",
    },
    {
      initials: "SS",
      name: "Smita Samanta",
...
/**
 * Display order for the single organisation grid on /team.
 *
 * `about` keeps founding partners, clients, and partners as separate arrays,
 * because /contact lists the founding partners and the preserved /about page
 * renders all three groups separately. /team flattens them into one grid, and
 * the order the CoLab wants there does not follow those groups — Tavily and
 * The Microsoft Garage are partners but sit next to a client, D_ID is a client
 * but sits after a partner.
 *
 * Rather than move organisations between the arrays (which would change what
 * /about and /contact claim about each relationship), the flat order is
 * declared here by name. Anything not listed keeps its existing relative
 * position and is appended, so adding an organisation without touching this
 * list still renders.
 */
const teamOrgOrder = [
...
/** Every organisation shown on /team, in the order above. */
export const teamOrgs: PartnerOrg[] = [
  ...about.foundingPartners,
  ...about.clients.items,
  ...about.partners.items,
].sort((a, b) => {
  const ia = teamOrgOrder.indexOf(a.name);
  const ib = teamOrgOrder.indexOf(b.name);
  // Unlisted organisations sort to the end, keeping their relative order.
